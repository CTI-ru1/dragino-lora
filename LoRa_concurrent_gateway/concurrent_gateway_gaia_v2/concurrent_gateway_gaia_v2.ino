...
//Polling periode
#define POLL_PERIODE 50  
#define POLL_NODE_PERIODE 50
...
{    
         ...
         for( b = 0;b < 200;b++)//loop to set up network
         {
            ...
            {  
              ...
              {    
                wdt_reset();             
                if(buf[0] == 'J' && buf[1] == 'R' && buf[2] == gateway_id)//Get a Join request, Store the Client ID
                  {                          
                    if(buf[3] == 0)// Error Message
                     ...
                     {
                       for(int b = 0;b < 5;b++)//Send 5 Join ACK to the Client,make sure it arrive.   MAX_BROADCAST_RETRY)
                       {
             ...
             {      
                 ...
                 if(CrcFlag == 1)//Calculate Successful, match
                 {
                    ...
                    if( buf[0] == 'D' && buf[1] == 'S' && buf[2] == gateway_id && buf[3] == query[3])
                    {
                        ...
                        if(sendMQTT(buf)==250)
                        {
                          //If the publish is corect increass the counter for  the specific node
                          pub[polling_count]++;
                        }
                        wdt_reset();
                        no_response_count = 0;
...
{   
     ...
     if(millis() - poll_start_time > REFRESH_TIME)//refresh the network 
     {
       ...
       for(i=0;i< client_numbers;i++)
       {
           ...
           sprintf(buf,"gaia %u/msg,%u+ret,%u+pub,%u+inv,%u+crc,%u+cyc,%u+\0",clients[i],msm[i],nret[i],pub[i],nmal[i],ncrc[i],ncyl);
          // sprintf(buf,"gaia");
           int i=0;
           while(i<sizeof(buf))
           {
