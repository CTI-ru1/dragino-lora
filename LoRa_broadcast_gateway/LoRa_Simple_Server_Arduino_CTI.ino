...
void sendMQTT(char * buf) {
  ...
  p.addParameter("....your IP....");  // Process that launch the "cat" command
  p.addParameter("-t");  // Process that launch the "cat" command
  p.addParameter(hostname);  // Process that launch the "cat" command
  ...
  while (p.available() > 0) {
   // Serial.println("Mosquito pub output");
    char c = p.read();
    Console.print(c);
  }
