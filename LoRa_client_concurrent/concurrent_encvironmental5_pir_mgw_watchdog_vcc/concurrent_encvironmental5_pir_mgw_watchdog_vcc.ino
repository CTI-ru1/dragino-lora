...
void read_sensors(String * message) {
  ...
  *message += "+s," + (String)sound;
  Serial.println(*message);
  memset(sensorvalue, 0, sizeof(sensorvalue));
  Serial.println(PIRValue);
  *message += "+p," + (String)PIRValue;
  PIRValue = 0;
  wdt_reset();

  long vcc=readVcc();
  char myvcc[10];
  sprintf(myvcc,"%lu",vcc);
  Serial.println("My vcc");
  Serial.println(myvcc);
  *message += "+v," +(String) myvcc + "+";
...
int check_sound()
{
  ...
  if((millis() - Soundtimestamp) > 50)
  {
    ...
    int newStatus = (int)(20*log10((maximum/1023.0*5.0))+85.0);
    if (newStatus==0)
    {
        sound=32;
    }
    ...
    {   
        sound=32;///if doesn not work put 32 db
    }   
    Soundtimestamp = millis();
  } 
  Serial.print("Sound");
