...
void read_sensors(int id, int rssi) {
  ...
  sprintf(bufst, "%d/t,%d.%02d+h,%d.%02d+l,%ld+s,%d+p,%d+v,%lu+r,%d+n,%d+", id, (int)tem, (int)(tem * 100) % 100, (int)hum, (int)(hum * 100) % 100, light, s, PIRValue, vcc, rssi,snr);
  PIRValue = 0;
  wdt_reset();
}
int check_sound()
